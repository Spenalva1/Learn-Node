  ...
  _axios2.default.get('/api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
    ...
    markers.forEach(function (marker) {
      return marker.addListener('click', function () {
        var html = '\n        <div class="popup">\n            <a href="/store/' + this.place.slug + '">\n                <img height=200 src="/uploads/' + (this.place.photo || 'store.png') + '" alt="' + this.place.name + '">\n                <p>' + this.place.name + ' - ' + this.place.location.address + '</p>\n            </a>\n        </div>\n        ';
...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    axios.get('/api/search?q=' + this.value).then(function (res) {
      if (res.data.length) return searchResults.innerHTML = _dompurify2.default.sanitize(searchResultsHtml(res.data));
  ...
  searchInput.on('keyup', function (e) {
    ...
    // KEYDOWN
    if (e.keyCode === 40) {
      var active = searchResults.querySelector('.search__result--active');
      if (!active) {
        return searchResults.firstElementChild.classList.add('search__result--active');
      }
    ...
    // KEYUP
    if (e.keyCode === 38) {
      var _active = searchResults.querySelector('.search__result--active');
      if (!_active) {
        return searchResults.lastElementChild.classList.add('search__result--active');
      }

      _active.classList.remove('search__result--active');

      var prev = _active.previousElementSibling;
      if (!prev) {
        return searchResults.lastElementChild.classList.add('search__result--active');
      }
    ...
    // ENTER
    if (e.keyCode === 13) {
      var _active2 = searchResults.querySelector('.search__result--active');
      if (!_active2) return;
      window.location.href = _active2.href;
    }
  });
}
